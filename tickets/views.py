...
import os
import shutil
from .models import Role, Ticket, Equipment, ServiceType, TicketStatus, TicketStatusHistory, Notification, User, LoginAttempt, IPBlock
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        is_blocked, block = IPBlock.check_ip_blocked(ip_address, user_agent)
        if is_blocked:
            messages.error(
                request,
                f'Ваш IP-адрес заблокирован из-за множества неудачных попыток входа. '
                f'Блокировка будет снята через {block.unblock_at - timezone.now()}'
            )
...
def unblock_ip(request, block_id):
    """Снятие блокировки IP администратором"""
    if not check_role(request.user, Role.ADMIN):
        return HttpResponseForbidden("Доступ запрещен")
    
    if request.method == 'POST':
        try:
            block = IPBlock.objects.get(id=block_id)
            block.is_active = False
            block.save()
            messages.success(request, f'Блокировка IP {block.ip_address} успешно снята')
        except IPBlock.DoesNotExist:
            messages.error(request, 'Блокировка не найдена')
    
    return redirect('tickets:security_panel')

@login_required
def security_panel(request):
    """Панель безопасности администратора с логированием входов и подозрительных действий"""
    ...
    if event_type_filter:
        login_attempts = login_attempts.filter(event_type=event_type_filter)
    
    if risk_level_filter:
        login_attempts = login_attempts.filter(risk_level=risk_level_filter)
    
    if suspicious_only:
        login_attempts = login_attempts.filter(is_suspicious=True)
    
    # Фильтр по дате
    try:
        days = int(days_filter)
        from_date = timezone.now() - timezone.timedelta(days=days)
        login_attempts = login_attempts.filter(timestamp__gte=from_date)
    except ValueError:
        days = 7
    ...
    login_attempts = login_attempts.order_by('-timestamp')
    
    # Пагинация
    paginator = Paginator(login_attempts, 20)  # 20 записей на странице
    login_attempts_page = paginator.get_page(page)
    
    # Статистика
    total_attempts = LoginAttempt.objects.count()
    successful_logins = LoginAttempt.objects.filter(event_type='login', is_successful=True).count()
    failed_logins = LoginAttempt.objects.filter(event_type='failed_login').count()
    suspicious_activities = LoginAttempt.objects.filter(is_suspicious=True).count()
    security_breaches = LoginAttempt.objects.filter(event_type='security_breach').count()
    
    # Уникальные IP адреса за последние 7 дней
    recent_ips_count = LoginAttempt.objects.filter(
        timestamp__gte=timezone.now() - timezone.timedelta(days=7)
    ).values('ip_address').distinct().count()
    
    # Уникальные пользователи за последние 7 дней
    recent_users_count = LoginAttempt.objects.filter(
        timestamp__gte=timezone.now() - timezone.timedelta(days=7),
        user__isnull=False
    ...
    context = {
        'login_attempts': login_attempts_page,
        'event_types': LoginAttempt.EVENT_TYPES,
        'risk_levels': [('low', 'Низкий'), ('medium', 'Средний'), ('high', 'Высокий'), ('critical', 'Критический')],
        ...
        'statistics': {
            ...
            'unique_ips_7days': recent_ips_count,
            'unique_users_7days': recent_users_count,
        },
        'active_blocks': active_blocks,
    }
    
    return render(request, 'admin_role/security_panel.html', context)
