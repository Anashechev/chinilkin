...
def validate_xss_protection(value):
    ...
    for pattern in dangerous_patterns:
        if re.search(pattern, value, re.IGNORECASE):
            raise ValidationError('Обнаружен потенциально опасный код. Пожалуйста, удалите скрипты, обработчики событий или JavaScript код.')
    
    # Экранирование HTML
    return escape(value)

class SafeFileField(forms.FileField):
    """Безопасное поле для загрузки файлов с валидацией MIME-типов"""
    
    def __init__(self, *args, **kwargs):
        self.allowed_extensions = kwargs.pop('allowed_extensions', None)
        self.max_file_size = kwargs.pop('max_file_size', 5 * 1024 * 1024)  # 5 МБ по умолчанию
        super().__init__(*args, **kwargs)
        
        if self.allowed_extensions:
            self.validators.append(FileExtensionValidator(allowed_extensions=self.allowed_extensions))
    
    def clean(self, data, initial=None):
        if data:
            # Проверка размера файла
            if data.size > self.max_file_size:
                raise ValidationError(f'Файл слишком большой. Максимальный размер: {self.max_file_size / (1024 * 1024):.1f} МБ')
            
            # Проверка расширения файла
            if self.allowed_extensions:
                file_ext = os.path.splitext(data.name)[1].lower()
                if file_ext not in self.allowed_extensions:
                    raise ValidationError(f'Недопустимое расширение файла. Разрешены: {", ".join(self.allowed_extensions)}')
...
class EquipmentForm(forms.ModelForm):
    ...
    def clean_model(self):
        ...
        if model:
            return validate_xss_protection(model)
        return model
    
    def clean_notes(self):
        notes = self.cleaned_data.get('notes')
        if notes:
            return validate_xss_protection(notes)
        return notes
...
class ServiceTypeForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if name:
            return validate_xss_protection(name)
        return name
    
    def clean_description(self):
        description = self.cleaned_data.get('description')
        if description:
            return validate_xss_protection(description)
        return description
